...
def generate_fio_fields(fio: str) -> dict:
...
1. Фамилия - только фамилия
2. Имя - только имя
3. Отчество - только отчество
4. Фамилия_инициалы - формат "Иванов И.П."
5. Фамилия_инициалы_рп - формат "Иванова И.П." (родительный падеж)
6. Фамилия_инициалы_дп - формат "Иванову И.П." (дательный падеж)
7. ФИО_рп - полное ФИО в родительном падеже "Иванова Ивана Петровича"
8. ФИО_дп - полное ФИО в дательном падеже "Иванову Ивану Петровичу"
9. ФИО_вп - полное ФИО в винительном падеже "Иванова Ивана Петровича" (кого?)

Верни ТОЛЬКО JSON без комментариев и markdown форматирования.

Пример для "Иванов Иван Петрович":
{{
  "Фамилия": "Иванов",
  "Имя": "Иван",
  "Отчество": "Петрович",
...
def save_debtor_data_only(debtor_id):
    ...
    try:
        ...
        # Если ФИО изменилось, генерируем все производные поля через нейронку
        if 'ФИО' in new_data and new_data['ФИО'] != current_data.get('ФИО'):
            print(f"[SAVE_DATA] ФИО changed, generating derivative fields via OpenAI...")
            fio_fields = generate_fio_fields(new_data['ФИО'])
