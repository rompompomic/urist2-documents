...
def _get_browser_channel():
    ...
    if system == "linux":
        ...
        import shutil
        if not (shutil.which("google-chrome") or shutil.which("chrome") or shutil.which("google-chrome-stable")):
            # If Chrome is not installed, fallback to bundled Chromium
            return None
...
class PatchrightScraper:
    ...
    def __init__(self, config: ScraperConfig | None = None):
        ...
        # Ensure a handler exists if not configured externally
    ...
    async def launch_persistent_context(
        ...
        try:
            # Note: For persistent context with patchright, we use chromium.launch_persistent_context
            # but specify channel='chrome' (via _get_browser_channel) to use the real installed Chrome if possible.
    ...
    async def fetch_content(
        ...
        Returns:
            List of (content, final_url) tuples from scraped pages
        """
        ...
        try:
            if handle_captcha:
                ...
                if pages:
                    ...
                    contents = [(first_page_content, first_page_url)]  # First page already scraped
                    ...
                    for page_num in page_numbers[1:]:
                        ...
                        contents.append((content, page.url))
                else:
                    contents = [(first_page_content, first_page_url)]
            else:
            ...
            contents = [(f"Error: {error_details}", url)]
        finally:
            if context:
                await context.close()
    ...
    async def scrape_multiple_pages(
            ...
            if use_persistent:
                ...
                try:
                    if self.config.use_stealth:
                        await self.apply_stealth_settings(page)
                    await self.set_browser_features(page)
                    return [await self.navigate_and_get_content(page, base_url)]
                finally:
                    await page.close()
            else:
                context = await self.create_context(browser, proxy)
        ...
        async def scrape_with_context(url: str, page_num: int) -> tuple[str, str]:
                ...
                if use_persistent:
                    ...
                    try:
                        if self.config.use_stealth:
                            await self.apply_stealth_settings(page)
                        await self.set_browser_features(page)
                        result = await self.navigate_and_get_content(page, url)
    ...
    async def navigate_and_get_content(self, page: Page, url: str) -> tuple[str, str]:
        ...
        try:
            ...
            return "Error: Timeout error", url
        except Exception as e:
            self.logger.error(f"Error navigating to {url}: {str(e)}")
            error_details = str(e) if len(str(e)) < 200 else str(e)[:200] + "..."
            return f"Error: Scraping failed\n\nDetails: {error_details}", url
