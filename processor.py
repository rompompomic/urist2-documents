...
ИЗВЛЕКАЙ ДАННЫЕ ТОЛЬКО ИЗ ДЕТАЛЬНЫХ БЛОКОВ! Краткую таблицу используй только для навигации.
...
2. **Вид**: Тип кредита из заголовка (Кредитная карта, Потребительский кредит, Овердрафт).
3. **Дата_сделки**: 
...
Верни только готовый пронумерованный список, без комментариев."""
    ...
    def process_pdf_with_text_extraction(self, pdf_path: Path, doc_type: str, base_prompt: str) -> tuple[Dict[str, Any], Optional[str]]:
        ...
        # Aggregate results
        # We need to deduplicate because chunks might overlap (though here they don't really overlap in content, just sequential)
        # But we must ensure correct structure.
        
        # Create final structure
        final_data = {
            "Кредиты": all_credits,
            # Add other fields if needed, but for credit reports "Кредиты" is the main one
            "Судебные_акты": [] # potentially from OKB
        }
