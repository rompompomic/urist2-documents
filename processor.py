...
"""
    ...
    def format_creditors_table(data_map: Dict[str, List[Dict[str, Any]]], debtor_fio: str = "", debtor_inn: str = "", debtor_snils: str = "", debtor_address: str = "") -> Dict[str, str]:
        ...
        # Ключ: кредитор + дата договора + сумма обязательства + текущий долг + тип кредита
        unique_credits = {}
        for credit in all_credits_list:
            # Формируем ключ с учетом всех важных параметров
            # Добавляем тип кредита и canonical имя для лучшей дедупликации
            creditor_key = credit['Кредитор_normalized'] or credit['Кредитор_canonical'] or credit['Кредитор']
            
            key = (
                creditor_key,
                credit['Дата_договора'],
                credit.get('Тип_кредита', ''),
