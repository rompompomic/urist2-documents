...
- НЕ группируй кредиты по названию банка. Один договор = один объект в JSON.
...
КРИТИЧЕСКИ ВАЖНО ПРО ИЗВЛЕЧЕНИЕ СПИСКА КРЕДИТОВ:
- Ты должен извлечь АБСОЛЮТНО ВСЕ КРЕДИТЫ (ДЕЙСТВУЮЩИЕ)!
- Если у должника несколько кредитов в одном банке (например, 5 кредитов в "АО «ГПБ»") — ЭТО РАЗНЫЕ КРЕДИТЫ! НЕ ОБЪЕДИНЯЙ ИХ!
- Создай отдельный элемент массива для КАЖДОГО кредитного договора.
- НЕ группируй кредиты по названию банка. Один договор = один объект в JSON.
- Точно перепиши суммы и даты для каждого кредита, чтобы они не перепутались.
...
- Если наткнешься на раздел с заголвком "Сведения о долговой нагрузке заемщика без заключенного потребительского займа" — прекращай сбор данных!
