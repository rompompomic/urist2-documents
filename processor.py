...
4. Штампы о семейном положении (стр. 14-15):
        ...
        СУПЕР-КРИТИЧЕСКОЕ ПРАВИЛО ПРО ВТОРОЙ ШТАМП:
        Если ты видишь нормальный штамп "Зарегистрирован брак", а ПОСЛЕ него (ниже или сбоку) есть ВТОРОЙ штамп, который:
        - Нечитаемый
        - Размытый / смазанный
        - Плохого качества
        - Невозможно прочитать слова "Брак расторгнут"
        ТО ТЫ ОБЯЗАН СЧИТАТЬ, ЧТО ЭТО ШТАМП О РАЗВОДЕ!
        В этом случае СТРОГО ставь статус: "Не женат" / "Не замужем".
        ...
        - ВЫВОД: Это развод. Статус -> "Не замужем". ФИО супруга -> null (или игнорируй).

        - КРИТИЧЕСКИ ВАЖНО: Если есть штамп о браке (и нет признаков развода/второго штампа) - извлеки ФИО супруга и дату рождения супруга из штампа!
