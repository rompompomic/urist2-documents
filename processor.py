...
1. Разворот с фото (стр. 2-3):
        ...
        - КРИТИЧЕСКИ ВАЖНО: Строка "Место рождения" находится СРАЗУ ПОД датой рождения НА СТРАНИЦЕ 2-3:
          ...
          * Если написано "Р.П. ЛЕБЯЖЬЕ ЛЕБЯЖЬЕВСКИЙ Р-Н КУРГАНСКАЯ ОБЛАСТЬ" - пиши так же
          * Если написано "СЕЛО ПЕТРОВО РЯЗАНСКАЯ ОБЛ." - пиши так же
          * Если написано "Гор. Миасс" - так и пиши, без изменений.
...
Записи должны быть отсортированы по дате (от старых к новым). Отсутствующие данные = null. Без комментариев.""",
        ...
        "счета": {
            "keywords": ["справка о счетах", "счета", "банков", "номер счета", "расчетный счет", "сведения о счетах"],
...
- ЕСТЬ хотя бы один кадастровый номер (формат: 74:19:1201002:4460 или XX:XX:XXXXXXX:XXXX)
...
- Имущество может быть в ипотеке, но она все равно в собственности, включай такое имущество.
- Если "дата государственной регистрации прекращения права: данные отсутствуют" — значит право действует, включай имущество.
...
СНАЧАЛА найди ИНН должника на первой странице отчета (это 12-значное число физического лица). Обычно указано в разделе "Сведения о субъекте" или "Информация о заемщике" в строке "ИНН".
...
JSON:
{{
  "ИНН_должника": "12-значное число физического лица из раздела 'Сведения о субъекте' на первой странице",
  "Кредиты": [
    {{
      "Кредитор": "название",
...
СНАЧАЛА найди ИНН должника на первой странице отчета (это 12-значное число физического лица). Обычно указано в разделе с информацией о заемщике в строке "ИНН".
...
{{
  "ИНН_должника": "12-значное число физического лица из раздела с информацией о заемщике на первой странице",
  "Кредиты": [
    {{
      "Кредитор": "название",
      "Вид": "тип кредита (Потребительский кредит/Кредитная карта/Микрозайм/Ипотека/Автокредит/другое)",
...
СНАЧАЛА найди ИНН должника на первой странице отчета (это 12-значное число физического лица). Обычно указано в разделе "Информация о субъекте кредитной истории" в строке "ИНН".
...
JSON (СТРОГО этот формат):
{{
  "ИНН_должника": "12-значное число физического лица из раздела 'Информация о субъекте кредитной истории' на первой странице",
  "Кредиты": [
    {{
      "Кредитор": "название",
...
- В этом случае верни: {"Результат": "транспорт отсутствует", "Является_справкой_ГИБДД": true}
...
- Отдел ФССП.
- Должник: ФИО.
- Взыскатель (кредитор): НАЗВАНИЕ организации или ФИО. ЕСЛИ НЕ УКАЗАНО ЯВНО в документе - ставь null!
...
Если реквизит не найден, ставь null. Никаких дополнительных комментариев.""",
        ...
        "свидетельство_о_расторжении_брака": {
            "keywords": ["свидетельство о расторжении брака", "свидетельство о разводе", "копия свидетельства о расторжении брака", "расторжение брака", "развод", "запись акта о расторжении брака"],
  ...
  "Название 2": "Нормализованное название 2"
    ...
    def format_real_estate_detailed(data_list: List[Dict[str, Any]], owner_fio: str = "", notification_list: Optional[List[Dict[str, Any]]] = None) -> str:
        ...
        if data_list:
            for data in data_list:
                ...
                if "квартир" in vid or "комнат" in vid:
                    continue
    ...
    def format_realty_table(егрн_docs: List[Dict[str, Any]], owner_fio: str = "") -> Dict[str, Any]:
        ...
        for doc in егрн_docs:
            ...
            # Формируем вид собственности с долей
            if доля and доля != "1/1":
                вид_собственности = f"{вид_права}. Доля в праве: {доля}" if вид_права else f"Доля в праве: {доля}"
            else:
                вид_собственности = вид_права or ""

            # Основание приобретения - только документ, БЕЗ доли
...
Верни только готовый пронумерованный список, без комментариев."""
    ...
    def prepare_template_context(data_map: Dict[str, List[Dict[str, Any]]], pdf_paths: Optional[List[Path]] = None, openai_client=None) -> Dict[str, str]:
        ...
        if not credit_history:
            credit_history = DocumentProcessor.select_first_entry(data_map, "отчет_бки")
...
Сразу нормализуй название банка, например: ПАО «Сбербанк», ПАО «Банк ПСБ», ПАО «ВТБ» и так далее.
