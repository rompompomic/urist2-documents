...
ПОЛЕ "Тип" - копируй вид счета ТОЧНО как в документе:
- Примеры: "Текущий счет", "Счет по вкладу", "Счет депозита", "Залоговый счет"
- Нормализуй регистр: первая буква заглавная, остальные строчные
- "ТЕКУЩИЙ СЧЕТ" → "Текущий счет", "СЧЕТ ПО ВКЛАДУ" → "Счет по вкладу"

Фильтр по статусу (включай только АКТИВНЫЕ):
- ✅ ВКЛЮЧАЙ: "открыт", "Текущий счет", "Счет по вкладу", "Предоставлено право использования"
- ❌ НЕ ВКЛЮЧАЙ: "закрыт", "Расторгнут", "Закрыт по инициативе клиента", "Прекращено право использования"
...
2️⃣ ПОИСК ОБЪЕКТОВ
   - Найди кадастровый номер (формат XX:XX:XXXXXXX:XXXX).
   - Если в документе есть таблица "Сведения об основных характеристиках объекта" с заполненными полями ("Площадь", "Кадастровая стоимость" и т.д.) — значит объект СУЩЕСТВУЕТ.

3️⃣ ИЗВЛЕЧЕНИЕ ДАННЫХ
   - Вид объекта: "Земельный участок", "Жилой дом", "Квартира" и т.д.
...
- Каждый объект имеет свой КАДАСТРОВЫЙ НОМЕР и свой НОМЕР РАЗДЕЛА
- ПРОВЕРЬ ВСЕ СТРАНИЦЫ документа - не останавливайся на первом объекте!
- Объекты нумеруются разделами: 1.X (первый объект), 2.X (второй объект), 3.X (третий объект) и т.д.
- Формат: "Объекты": [объект1, объект2, объект3, ...]
- Извлеки КАЖДЫЙ объект со ВСЕМИ его характеристиками

СТРУКТУРА РАЗДЕЛОВ ДЛЯ КАЖДОГО ОБЪЕКТА:
- ПЕРВЫЙ объект: разделы 1.1, 1.2, 1.3
- ВТОРОЙ объект: разделы 2.1, 2.2, 2.3  
- ТРЕТИЙ объект: разделы 3.1, 3.2, 3.3
- И так далее...

ПРИМЕР: если видишь разделы 1.1, 1.2, 1.3, потом 2.1, 2.2, 2.3, потом 3.1, 3.2, 3.3 → это 3 РАЗНЫХ объекта!

АЛГОРИТМ ПРОВЕРКИ СТАТУСА ПРАВА (включая Госуслуги):
1. Найди поле с датой прекращения права (варианты названий):
   - "Дата, номер и основание государственной регистрации прекращения права"
   - "Дата, номер и основание государственной регистрации перехода (прекращения) права"
   - "Дата государственной регистрации прекращения права"
   - "Сведения о прекращении права"

2. Проверь значение этого поля:
   - Если содержит "данные отсутствуют", "не зарегистрировано", "нет" → ПРАВО АКТИВНО (включить в "Объекты")
   - Если содержит "Право на недвижимость действующее" или "активно" → ПРАВО АКТИВНО (включить в "Объекты")
   - Если содержит конкретную дату (например, "01.01.2023") И НЕ содержит слов "действующее"/"активно" → ПРАВО ПРЕКРАЩЕНО (включить в "Объекты" удалив пометку активности или добавить в Сделки)
   
3. Если строка пустая или отсутствует → ПРАВО АКТИВНО.
...
- Если в разделе 1.2 "Дата государственной регистрации прекращения права" стоит дата И нет слов "действующее" → право ПРЕКРАЩЕНО, это сделка
- Включай записи где "Дата..." = "данные отсутствуют" ИЛИ "Право на недвижимость действующее"
- Если права изменились (продажа, дарение) — это СДЕЛКА, извлекай её в раздел "Сделка"
