...
- Если видите "Пенсия по инвалидности" или подобные, по старости например - обязательно извлеките сумму.
        ...
        for doc in pension_data:
            ...
            for benefit in benefits:
                ...
                if benefit_type:
                    ...
                    # Игнорируем технические строки с итогами, чтобы не дублировать доход
                    if "суммарный размер" in benefit_lower or "размер фиксированной выплаты" in benefit_lower:
                        continue
                    ...
                    if "едв" in benefit_lower or "ежемесячная денежная выплата" in benefit_lower:
                        ...
                        # Пенсия должна отображаться в иные доходы, если нет отдельного поля
                        desc = f"Должник получает пенсию ({benefit_type})"
                    else:
            ...
            for payment in payments:
                ...
                if payment_name:
                    ...
                    if "пенси" in payment_lower:
                        # Проверяем, есть ли уже такая запись (из блока пенсий), чтобы не дублировать
                        # Хотя тут "выплаты" - это фактические выплаты, а там - назначенные
                        # Но лучше добавить
