...
4. Штампы о семейном положении (стр. 7-8):
        ...
        - Если штамп зачеркнут, перечеркнут, наложен крест — значит развод, статус "Разведен/Разведена".
...
Пустые поля = null, списки допускают [] и null, но ключи обязательны. Без комментариев.""",
        ...
        "налоговое_уведомление": {
            "keywords": ["налоговое уведомление", "налогового уведомления", "налоговое уведомлени", "копия налогового", "транспортный налог", "налог на имущество физических лиц", "всего к уплате", "земельный налог", "фнс", "налоговая служба"],
...
Для т-банка сразу в json подставь адрес "127287, город Москва, ул Хуторская 2-Я, д. 38а стр. 26"

КРИТИЧЕСКОЕ ПРАВИЛО ИСКЛЮЧЕНИЯ (ПОСТАНОВЛЕНИЯ / ИСПОЛНИТЕЛЬНЫЕ ДОКУМЕНТЫ):

Если в детальном разделе договора выполняется ХОТЯ БЫ ОДНО из условий ниже,
ДОГОВОР НУЖНО ПОЛНОСТЬЮ ПРОПУСТИТЬ и НЕ добавлять в JSON:

1. В таблице "Сведения о сумме задолженности" присутствует строка:
   "Сумма задолженности на дату передачи финансирования субъекту
    или возникновения обеспечения исполнения обязательства".

2. В детальном разделе присутствуют признаки взыскания или исполнительного документа,
...
4. "Сумма и валюта обязательства" равна 0 р. на дату последнего обновления.

❗️ Такие записи считать НЕ кредитами, а постановлениями / исполнительными документами.
❗️ Эти записи обрабатываются ОТДЕЛЬНО и НЕ должны попадать в результат.
...
🔴 КРИТИЧЕСКИ ВАЖНО - ИТОГОВАЯ СУММА ВЗЫСКАНИЯ:

В постановлении могут быть НЕСКОЛЬКО сумм:
1. Основной долг (основная сумма долга) - например: 96391,98 руб
2. Проценты - например: 48815,70 руб
3. Расходы на нотариальные действия - например: 1558,00 руб
4. Исполнительский сбор - например: 7% от суммы
5. **ИТОГО ВЗЫСКАНИЕ** ← САМОЕ ВАЖНОЕ ПОЛЕ!

АЛГОРИТМ ПОИСКА ИТОГОВОЙ СУММЫ:
1. Ищи строку со словами "Итого" или "Всего взыскать" в документе
2. Примеры фраз:
   - "Итого, по настоящей исполнительной надписи взыскать 146765,68 рублей"
   - "Итого взыскание: 146 765.68 руб"
   - "Всего взыскать: 146765,68"
   - "Подлежит взысканию: 146 765,68 рублей"
3. Извлеки число ПОСЛЕ слова "Итого"/"Всего" - это и есть "Итого_взыскание"
4. Если несколько сумм "Итого" - бери ПОСЛЕДНЮЮ (финальную)

ПРАВИЛО:
"Итого_взыскание" = ВСЯ сумма, которую должен заплатить должник по постановлению
Если "Итого" написано явно - используй ЭТУ цифру, НЕ складывай части!
...
JSON:
{
        ...
        "Взыскатель": "ПАО «ВТБ»",
        "Адрес_взыскателя": "191144, Санкт-Петербург, пер Дегтярный, д 11 литер А",
        "Предмет_исполнения": "Взыскание задолженности по исполнительной надписи",
        ...
        "Адрес_органа": "455045, Челябинская обл, г Магнитогорск, ул Бориса Ручьева, д 1А",
...
ПРИМЕР: "Итого, по настоящей исполнительной надписи взыскать 146765,68 рублей"
→ Итого_взыскание = 146765.68
...
"""
    ...
    def format_creditors_table(data_map: Dict[str, List[Dict[str, Any]]], debtor_fio: str = "", debtor_inn: str = "", debtor_snils: str = "", debtor_address: str = "") -> Dict[str, str]:
        ...
        for постановление in data_map.get("постановление_пристава", []):
            ...
            # Проверяем, есть ли уже кредитор с таким нормализованным названием
            if взыскатель_normalized in credits_by_creditor:
                ...
                print(f"[ПОСТАНОВЛЕНИЕ] Новый кредитор: {взыскатель_canonical}")
                other_debts.append({
                    "Кредитор": взыскатель,
                    "Кредитор_canonical": взыскатель_canonical,
                    "Кредитор_normalized": взыскатель_normalized,
        ...
        for debt in other_debts:
            ...
            # ИНН кредитора
            инн_display = DocumentProcessor.get_bank_inn(кредитор_canonical)
            if инн_display:
                инн_display = f"ИНН {инн_display}"
...
1. В разделе "Сумма и валюта обязательства" присутствуют значения "0 р."
   (на любую дату после даты возникновения обязательства).

2. В таблице "Сведения о сумме задолженности" присутствует строка:
   "Сумма задолженности на дату передачи финансирования субъекту
    или возникновения обеспечения исполнения обязательства".

3. В детальном разделе присутствуют признаки взыскания или исполнительного документа,
   включая (но не ограничиваясь):
   — "исполнительная надпись"
   — "исполнительное производство"
   — "ФССП"
