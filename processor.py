...
1️⃣ ПРОЧИТАЙ ВЕСЬ ДОКУМЕНТ ОТ ПЕРВОЙ ДО ПОСЛЕДНЕЙ СТРАНИЦЫ
   - НЕ останавливайся после нахождения первого кадастрового номера
   - НЕ пропускай страницы в начале или середине документа
   - Квартира с продажей обычно идет ПЕРВОЙ в документе

2️⃣ НАЙДИ ВСЕ КАДАСТРОВЫЕ НОМЕРА (формат XX:XX:XXXXXXX:XXXX)
   - Ищи строки "Кадастровый номер:", "кадастровый №", "Кадастровый №"
   - Каждый УНИКАЛЬНЫЙ номер = отдельный объект
   - ПРИМЕР: Видишь номера 74:36:0212004:1852, 74:36:0312023:1548, 74:36:0312023:3050 → это 3 ОБЪЕКТА!

3️⃣ ОПРЕДЕЛИ НОМЕРА РАЗДЕЛОВ ДЛЯ КАЖДОГО ОБЪЕКТА
   - Первый кадастровый номер → разделы 1.1, 1.2, 1.3 (характеристики, правообладатели, обременения)
   - Второй кадастровый номер → разделы 2.1, 2.2, 2.3
   - Третий кадастровый номер → разделы 3.1, 3.2, 3.3
   - И так далее...

4️⃣ ИЗВЛЕКИ ДАННЫЕ КАЖДОГО ОБЪЕКТА
   - Из раздела X.1: вид объекта, адрес, площадь, кадастровая стоимость
   - Из раздела X.2: правообладатели с датами регистрации и прекращения права
   - Из раздела X.3: обременения (ипотека, залог)

КОНКРЕТНЫЙ ПРИМЕР:
Документ содержит:
Раздел 1.1, 1.2, 1.3 → Квартира (кадастровый 74:36:0212004:1852) → ПРОДАНА 02.02.2023
Раздел 2.1, 2.2, 2.3 → Участок (кадастровый 74:36:0312023:1548) → куплен 28.02.2023
Раздел 3.1, 3.2, 3.3 → Здание (кадастровый 74:36:0312023:3050) → куплено 23.03.2023

РЕЗУЛЬТАТ: {"Есть_недвижимость": true, "Объекты": [квартира, участок, здание]}

⚠️ ЧАСТАЯ ОШИБКА: Пропуск объекта (квартиры с продажей)!
   - НЕ пропускай объекты
   - Даже если квартира продана (есть дата прекращения права) - ВКЛЮЧАЙ её в массив "Объекты"
   - Проверь: сколько кадастровых номеров нашел = столько объектов должно быть в массиве

ШАГ 1 - ОПРЕДЕЛИ КОЛИЧЕСТВО ОБЪЕКТОВ:
ПЕРВЫМ ДЕЛОМ найди ВСЕ кадастровые номера в документе (формат XX:XX:XXXXXXX:XXXX или XX:XX:XXXXXXX:XX).
Сколько РАЗНЫХ кадастровых номеров → столько объектов недвижимости!
Примеры:
- Нашел кадастровые номера: 74:36:0212004:1852, 74:36:0312023:1548, 74:36:0312023:3050 → 3 объекта
- Нашел один номер 50:21:0020301:45 → 1 объект
- Не нашел ни одного кадастрового номера → имущества нет

ШАГ 2 - НАЙДИ РАЗДЕЛЫ ДЛЯ КАЖДОГО ОБЪЕКТА:
Каждый кадастровый номер имеет свои разделы X.1, X.2, X.3 (где X = номер объекта):
- Первый кадастровый номер → разделы 1.1, 1.2, 1.3
- Второй кадастровый номер → разделы 2.1, 2.2, 2.3
- Третий кадастровый номер → разделы 3.1, 3.2, 3.3

ШАГ 3 - ИЗВЛЕКИ ДАННЫЕ ИЗ КАЖДОГО РАЗДЕЛА:
Для КАЖДОГО найденного кадастрового номера извлеки:
- Из X.1: вид объекта, адрес, площадь, кадастровая стоимость
- Из X.2: правообладатели с датами регистрации и прекращения права
- Из X.3: обременения (ипотека, залог)

ПРИМЕР: Если нашел 3 кадастровых номера → должно быть 3 объекта в массиве "Объекты"!

УВЕДОМЛЕНИЕ ОБ ОТСУТСТВИИ (имущества НЕТ):
- Фразы: "отсутствует запрошенная Вами информация", "в ЕГРН отсутствуют сведения"
- НЕТ кадастровых номеров вообще
- Таблица ПУСТАЯ или содержит только "Вид запрошенной информации" БЕЗ данных
- Только в этом случае: {"Есть_недвижимость": false, "Причина": "Уведомление об отсутствии"}

ВЫПИСКА С ИМУЩЕСТВОМ (имущество ЕСТЬ):
- Есть таблица с характеристиками объекта
- Есть раздел с правообладателями и их ФИО
- ОБЯЗАТЕЛЬНО: {"Есть_недвижимость": true, "Объекты": [...]}
...
- Каждая ЕДИНИЦА НЕДВИЖИМОСТИ имеет разделы с номером объекта:
  * X.1 - Характеристики объекта (вид, кадастровый номер, назначение, местоположение, площадь)
  * X.2 - Сведения о правообладателях (вид права, доля, даты регистрации, документ-основание)
  * X.3 - Ограничения и обременения (ипотека, залог, арест и т.д.)
  где X = 1, 2, 3... (номер объекта)
  
ПРИМЕРЫ:
- Один объект: разделы 1.1, 1.2, 1.3
- Два объекта: разделы 1.1, 1.2, 1.3 + 2.1, 2.2, 2.3
- Три объекта: разделы 1.1, 1.2, 1.3 + 2.1, 2.2, 2.3 + 3.1, 3.2, 3.3
...
3. ГОСУДАРСТВЕННЫЙ НОМЕР:
   ...
   - В ПТС: Государственный регистрационный знак
...
Если реквизит не найден, ставь null. Никаких дополнительных комментариев.""",
        ...
        "свидетельство_о_браке": {
            ...
            "prompt": """Ты обрабатываешь свидетельство о заключении брака (РФ или других стран СНГ, например Таджикистана).
...
🔴🔴🔴 КРИТИЧЕСКИ ВАЖНО - СТРУКТУРА ДОКУМЕНТА:

Свидетельство о браке обычно состоит из ДВУХ частей:
1. **РУКОПИСНАЯ ЧАСТЬ** (верх документа, первая страница):
   - Заполнена от руки
   - Может быть нечитаемой, размытой, со сложным почерком
   - ЕСЛИ НЕ МОЖЕШЬ ПРОЧИТАТЬ РУКОПИСНЫЙ ТЕКСТ - НЕ ПЫТАЙСЯ! ИДИ К ПЕЧАТНОЙ ЧАСТИ!

2. **ПЕЧАТНАЯ ЧАСТЬ** (низ документа или следующая страница):
   - Печатный дубликат той же информации
   - ЯВНО УКАЗАНЫ ГРАФЫ: "муж:", "жена:", "мужу", "жене"
   - Легко читается
   - **ПРИОРИТЕТ: ВСЕГДА ИСПОЛЬЗУЙ ПЕЧАТНУЮ ЧАСТЬ!**

🎯 АЛГОРИТМ:

ЭТАП 1 - НАЙДИ ПЕЧАТНУЮ ЧАСТЬ:
- Пролистай ВСЕ страницы документа от начала до конца
- Ищи блок с печатным текстом (обычно внизу или на 2-й странице)
- Ищи явные маркеры: "муж:", "жена:", "мужу присвоена", "жене присвоена"
- Пример структуры печатной части:
  ```
  Сочетались браком:
  муж: ИВАНОВ ИВАН ИВАНОВИЧ, 01.01.1990
  жена: ПЕТРОВА МАРИЯ СЕРГЕЕВНА, 02.02.1992
...
ЭТАП 2 - ИЗВЛЕКИ ДАННЫЕ ИЗ ПЕЧАТНОЙ ЧАСТИ:
- Найди строку с "муж:" или "мужу" → извлеки ФИО и дату рождения мужа
- Найди строку с "жена:" или "жене" → извлеки ФИО и дату рождения жены
- Найди "присвоены фамилии" → возьми АКТУАЛЬНЫЕ фамилии после брака

ЭТАП 3 - ОПРЕДЕЛИ ДОЛЖНИКА:
- Сравни {debtor_fio} с ФИО из графы "муж:"
- Сравни {debtor_fio} с ФИО из графы "жена:"
- Сравнивай ТОЛЬКО фамилию + имя (отчество может отличаться)
- Игнорируй регистр букв

ЭТАП 4 - ВЕРНИ СУПРУГА:
- Если должник найден в графе "муж:" → верни данные из графы "жена:"
- Если должник найден в графе "жена:" → верни данные из графы "муж:"
- Используй АКТУАЛЬНУЮ фамилию после брака!

ПРИМЕР:

{debtor_fio} = "Мансуров Ахмад Тоджидинович"

Печатная часть документа:
```
Сочетались браком:
муж: МАНСУРОВ АХМАД ТОДЖИДИНОВИЧ, 14.01.1997
жена: ШАХОВБИЛЛИНЗОДА БИБИРАДЖАБ ШОМУРОД, 06.08.1999

Присвоены фамилии:
мужу: МАНСУРОВ
жене: ШАХОВБИЛЛИНЗОДА
```

Анализ:
1. Графа "муж:" содержит "МАНСУРОВ АХМАД" ✅ = должник
2. Графа "жена:" содержит другое лицо → это СУПРУГ должника
3. Актуальная фамилия жены = ШАХОВБИЛЛИНЗОДА

Результат:
```json
{
  "Супруг_ФИО": "Шаховбиллинзода Бибираджаб Шомурод",
  "Супруг_Дата_рождения": "06.08.1999"
}
```
  ...
  "Название 2": "Нормализованное название 2"
        ...
        for doc in егрн_docs:
            ...
            for obj in объекты_список:
                ...
                if тип_собственности:
                    части_собственности.append(тип_собственности)
    ...
    def format_vehicles_table(гибдд_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for doc in deduplicated:
            ...
            # Пропускаем если нет марки и VIN (пустой документ)
            if not марка and not вин:
                print(f"  [SKIP] Пропущено: нет марки и VIN")
                continue
