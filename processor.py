...
🔴 КРИТИЧЕСКИ ВАЖНО - ИЗВЛЕКАЙ ВСЕ СЧЕТА:
- Внимательно просмотри ВЕСЬ документ от начала до конца и найди ВСЕ счета!
- Читай таблицу построчно до самого конца документа, не пропускай строки!
- ОБЯЗАТЕЛЬНО включай: текущие счета, счета по вкладу, депозитные счета, карточные счета
- Если в справке 5 счетов - извлеки все 5! Если 10 счетов - извлеки все 10! Если 20 - все 20!
- Один банк может иметь НЕСКОЛЬКО счетов (например: Сбербанк текущий в Москве + Сбербанк вклад в Челябинске + Сбербанк депозит)
...
Фильтр по статусу:
- Извлекай ВСЕ счета, которые видишь в таблице, независимо от статуса (открыт/закрыт).
- Статус ("открыт", "закрыт", "действующий") запиши в поле "Статус".
...
В массив "Счета" включай ВСЕ найденные счета (и активные, и закрытые). Статус счета указывай в поле "Статус". Если данных нет — null, списки допускают []. Никаких комментариев.""",
        },
        "_уведомлениегрне": {
...
- Комментарии не добавляй"""
        ...
        "пенсии": {
            "keywords": ["справка сфр", "назначенных пенсиях", "ежемесячной денежной выплаты", "едв", "пенсия", "о назначенных пенсиях", "социальных выплатах", "размер пенсии", "пенсионный фонд", "пфр", "справка сфр о назначенных пенсиях", "справка сфр"],
...
"""
    ...
    def get_bank_address(bank_name: str, inn: Optional[str] = None) -> str:
        ...
        BANK_ADDRESSES = {
            # Крупнейшие банки
            "СБЕРБАНК": "117312, город Москва, ул. Вавилова, д.19", 
            "ВТБ": "191144, город Санкт-Петербург, Дегтярный пер., д. 11 литер а", 
            "АЛЬФА БАНК": "107078, город Москва, Каланчевская ул., д.27", 
            "ТИНЬКОФФ": "127287, г. Москва, ул. Хуторская 2-я, д. 38А, стр. 26", 
            "Т БАНК": "127287, г. Москва, ул. Хуторская 2-я, д. 38А, стр. 26",
            "ТБАНК": "127287, г. Москва, ул. Хуторская 2-я, д. 38А, стр. 26",
            "СОВКОМБАНК": "156000, Костромская область, г. Кострома, пр-кт Текстильщиков, д. 46",
            "ПОЧТА БАНК": "107061, г. Москва, пл. Преображенская, д. 8",
            "ХОУМ БАНК": "125040, г. Москва, ул. Правды, д. 8, корп. 1",
            "ОТП БАНК": "125171, г. Москва, ш. Ленинградское, д. 16А, стр. 1",
            "РЕНЕССАНС КРЕДИТ": "115114, г. Москва, наб. Дербеневская, д. 7, стр. 22",
            "МТС БАНК": "115432, г. Москва, пр-кт Андропова, д. 18, корп. 1",
            "РУССКИЙ СТАНДАРТ": "105187, г. Москва, ул. Ткацкая, д. 36",
            "УРАЛСИБ": "119048, г. Москва, ул. Ефремова, д. 8",
            
            # МФО (частые)
            "МАНИМЕН": "121096, город Москва, ул. Василисы Кожиной, д. 1, офис д13",
            "ЗАЙМЕР": "630112, Новосибирская область, г. Новосибирск, ул. Фрунзе, д. 242, офис 406", 
            "ЕКАПУСТА": "630055, Новосибирская область, г. Новосибирск, ул. Гнесиных, д. 10/1, офис 202", 
            "СМСФИНАНС": "123112, город Москва, Пресненская наб, д. 12, этаж 67 комн. 8",
            "ВЭББАНКИР": "125466, город Москва, ул. Соколово-Мещерская, д. 29, офис 308",
            "МИГКРЕДИТ": "127018, город Москва, ул. Сущёвский Вал, д. 5, стр. 3", 
            "БЫСТРОДЕНЬГИ": "123290, город Москва, туп. 1-й Магистральный, д. 11, стр. 10", 
            "ВЕРИТАС": "115114, Г.Москва, ВН.ТЕР.Г. МУНИЦИПАЛЬНЫЙ ОКРУГ ДАНИЛОВСКИЙ, НАБ ДЕРБЕНЕВСКАЯ, Д. 11, ПОМЕЩ. 13/2",
            "ЕЗАЕМ": "115114, Г.Москва, ВН.ТЕР.Г. МУНИЦИПАЛЬНЫЙ ОКРУГ ДАНИЛОВСКИЙ, НАБ ДЕРБЕНЕВСКАЯ, Д. 11, ПОМЕЩ. 13/2",
            "ТУРБОЗАЙМ": "123290, город Москва, туп 1-й Магистральный, д. 11 стр. 10, офис 101",
            "КВИКУ": "119019, город Москва, ул. Новый Арбат, д. 15, стр. 1",
            "ЛЮК": "119019, город Москва, ул. Новый Арбат, д. 15, стр. 1",
            "ДЕНЬГИ СРАЗУ": "344034, Ростовская область, г. Ростов-на-Дону, ул. Портовая, д. 193, офис 309", 
            "КРЕДИТТЕР": "115280, город Москва, ул. Ленинская Слобода, д. 19, стр. 6, ком. 7", 
            "JOYMONEY": "630099, Новосибирская область, г. Новосибирск, ул. Советская, д. 12",
            "ОТЛИЧНЫЕ НАЛИЧНЫЕ": "630055, Новосибирская область, г Новосибирск, ул Гнесиных, д. 10/1, офис 202", 
            "ПЛАТИЗА": "109316, город Москва, пр-кт Волгоградский, д. 43, корп. 3, эт 11 ком 10",
            
            # Новые добавленные
            "ОПТИМУМ ФИНАНС": "119435, город Москва, Большой Саввинский пер, д. 10 стр. 2 а",
            "СФО ОПТИМУМ ФИНАНС": "119435, город Москва, Большой Саввинский пер, д. 10 стр. 2 а",
            "УК ТРАСТ": "197374, город Санкт-Петербург, Мебельная ул, д. 12 к. 1 литера А, помещ. 55-н офис 415-6",
        }
            ...
            if items:
                for item in items:
                    ...
                    if is_inn_search:
                        ...
                        if len(query_abbr_parts) > 1 and query_abbr_parts[0].upper() in ["МКК", "МФК", "МФО", "ООО", "АО", "ПАО"]:
...
{{
  ...
  "Название 2": "Нормализованное название 2"
    ...
    def get_marital_status_key(passport: Dict[str, Any], data_map: Dict[str, List[Dict[str, Any]]] = None) -> str:
        ...
        not_married = "не женат" in marital_status or "не замужем" in marital_status
        ...
        for doc in егрн_docs:
            ...
            for obj in объекты_список:
                ...
                for право in правообладатели:
                    ...
                    дата_clean = str(дата_прекращения).strip().lower()
                    if not дата_clean or дата_clean in ["null", "none", "нет", "отсутствует", "данные отсутствуют", "-"]:
                        has_active_ownership = True
                        break
    ...
    def format_vehicles_table(гибдд_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Возвращаем списки транспортных средств как есть (каждое ТС - отдельный элемент)
        # Шаблон использует циклы {% for авто in автомобили %} для каждого типа
        return {
...
Верни только готовый пронумерованный список, без комментариев."""
    ...
    def process_pdf_with_text_extraction(self, pdf_path: Path, doc_type: str, base_prompt: str) -> tuple[Dict[str, Any], Optional[str]]:
        ...
        print(f"      [TEXT] Извлечение текста с помощью PyMuPDF...")
