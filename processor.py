...
1. Разворот с фото (стр. 2-3):
        ...
        - КРИТИЧЕСКИ ВАЖНО: Строка "Место рождения" находится СРАЗУ ПОД датой рождения:
...
3. Разворот регистрации (стр. 5-6):
        - ВНИМАНИЕ: Если несколько записей о регистрации — выбери ПОСЛЕДНЮЮ (с самой поздней датой).
        ...
        - КРИТИЧЕСКИ ВАЖНО: Адрес регистрации может быть в ШТАМПЕ или рукописной записи:
          ...
          * В старых штампах часто есть "г. ЧЕЛЯБИНСК", "УЛ. КРАСНОМОЛОТОВА" и т.д.
...
Верни JSON строго в формате:
{
  ...
  "Место_рождения": "с. Агинское, Саянский р-н, Красноярский край",  # ОБЯЗАТЕЛЬНОЕ ПОЛЕ! Формат: населенный пункт, район, область/край (через запятую, с сокращениями)
  "Серия": "ХХХХ",
  "Номер": "XXXXXX",
  "Кем_выдан": "...",
...
КРИТИЧЕСКИ ВАЖНО: Поле "Место_рождения" ОБЯЗАТЕЛЬНО должно быть заполнено в формате: "тип населенного пункта (с сокращением), название, район, область/край". Примеры: "с. Агинское, Саянский р-н, Красноярский край", "г. Москва", "д. Петрово, Рязанская обл.". Если текст нечитабелен — напиши что видишь, но НЕ оставляй null!
...
"""
    ...
    def format_vehicles_table(гибдд_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        Returns:
            Словарь с ключом "автомобили" (список для Jinja2)
        """
        автомобили = []
        counter = 1
    ...
    def get_arbitration_court(address: str) -> str:
        ...
        prompt = (
            ...
            "- Для областей: 'Арбитражный суд Челябинской области', 'Арбитражный суд Свердловской области'\n"
            "- Для краев: 'Арбитражный суд Краснодарского края', 'Арбитражный суд Красноярского края'\n"
            "- Для республик: 'Арбитражный суд Республики Башкортостан', 'Арбитражный суд Республики Татарстан'\n\n"
            f"Адрес регистрации: {address}\n\n"
            "Пример 1: Челябинская область → Арбитражный суд Челябинской области\n"
            "Пример 2: г. Москва → Арбитражный суд города Москвы\n"
            "Пример 3: Краснодарский край → Арбитражный суд Краснодарского края"
        )
