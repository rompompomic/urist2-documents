...
Сразу нормализуй название банка, например: ПАО «Сбербанк», ПАО «Банк ПСБ» и т.д.
...
ФОРМАТИРОВАНИЕ: Если текст в CAPS LOCK — приведи к нормальному виду.
Сразу нормализуй название банка: ПАО «Сбербанк», ПАО «Банк ПСБ», ПАО «ВТБ».
    ...
    def format_creditors_table(data_map: Dict[str, List[Dict[str, Any]]], debtor_fio: str = "", debtor_inn: str = "", debtor_snils: str = "", debtor_address: str = "") -> Dict[str, str]:
        ...
        for кредитор_normalized, credits_list in sorted_creditors:
            ...
            for contract_idx, credit in enumerate(credits_list):
                ...
                # Очищаем тип кредита от суммы в рублях (если есть)
                # Примеры: "Задолженность по кредитным платежам (кроме ипотеки) в размере: 73 268.41 руб."
                # Нужно оставить только: "Задолженность по кредитным платежам (кроме ипотеки)"
                import re
                # Удаляем " в размере: <число> руб." и похожие паттерны
                ...
                table_rows.append({
                    ...
                    "ИНН_кредитора": инн_display,  # Показываем на всех строках
        ...
        for doc in егрн_docs:
            ...
            for obj in объекты_список:
                ...
                for право in правообладатели:
                    ...
                    if not дата_прекращения or дата_прекращения == "данные отсутствуют":
                        has_active_ownership = True
                        break
    ...
    def prepare_template_context(data_map: Dict[str, List[Dict[str, Any]]], pdf_paths: Optional[List[Path]] = None, openai_client=None) -> Dict[str, str]:
        ...
        if devichya_familiya and owner_fio:
            ...
            if len(fio_parts) >= 3:
                ...
                if "(" not in owner_fio:
                    owner_fio = f"{fio_parts[0]} ({devichya_familiya}) {' '.join(fio_parts[1:])}"
