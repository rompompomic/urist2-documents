...
Как только встретишь таблицу "ЗАКРЫТЫЕ КРЕДИТНЫЕ ДОГОВОРЫ", полностью прекращай обработку договоров и игнорируй всё ниже.

В разделе "ДЕЙСТВУЮЩИЕ КРЕДИТНЫЕ ДОГОВОРЫ":
— вверху есть краткая таблица;
— НИЖЕ идёт детальное описание договоров (1., 2., 3. и т.д.).

НЕ используй краткую таблицу для извлечения детальных данных.
Всегда используй ТОЛЬКО детальный раздел НИЖЕ соответствующего кредитора.

Сразу нормализуй название банка (например: ПАО «Сбербанк», ПАО «Банк ПСБ» и т.д.).
Для АО «ТБАНК» (Т-Банк) всегда подставляй адрес:
"127287, город Москва, ул Хуторская 2-Я, д. 38а стр. 26".

ДЛЯ КАЖДОЙ строки в таблице "ДЕЙСТВУЮЩИЕ КРЕДИТНЫЕ ДОГОВОРЫ":

Если в колонке "Сумма и валюта обязательства" указано "0" —
НЕ добавляй этот договор в результат (это постановление или иной документ, долг уже учтён).

Для остальных строк:

1. Возьми название кредитора из краткой таблицы.
2. Найди соответствующий детальный раздел НИЖЕ с этим кредитором и извлеки:

   • Дата сделки — поле "Дата совершения сделки"
     (если отсутствует — "Дата возникновения обязательства").

   • Сумма и валюта обязательства —
     ПЕРВОНАЧАЛЬНАЯ сумма кредита из детального раздела
     (первое значение по дате возникновения обязательства).

   • В разделе "Сведения об источнике" найди блок
     "ИНН и ОГРН (ОГРНИП)":
       – извлеки ТОЛЬКО ИНН из строки с пометкой "ИНН³"
       – ИНН всегда состоит ровно из 10 цифр
       – ОГРН ИГНОРИРОВАТЬ

   • Тип кредита/займа —
     брать из детального раздела ("Вид займа (кредита)" или аналогичное поле):
       Потребительский кредит / Кредитная карта / Микрозайм / Ипотека /
       Автокредит / Необеспеченный кредит / Другое

КРИТИЧЕСКИ ВАЖНО ПРО ДОЛГ (ОБЯЗАТЕЛЬНО СОБЛЮДАТЬ):

Поле "Сумма" в JSON рассчитывается ТОЛЬКО по ТАБЛИЦЕ
"Сведения о сумме задолженности" и НИКАК ИНАЧЕ.

КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО:
— использовать ЛЮБЫЕ строки, содержащие двоеточие ":"
— использовать строки, которые НЕ содержат колонок:
  "Основной долг", "Проценты", "Иное"
— использовать строку:
...
❗ Такие записи считать НЕ кредитами, а постановлениями / исполнительными документами.
❗ Эти записи обрабатываются ОТДЕЛЬНО и НЕ должны попадать в результат.

АЛГОРИТМ РАСЧЁТА "Сумма":

1. Найди таблицу "Сведения о сумме задолженности".
2. ПРОПУСТИ все строки до заголовка колонок:
   "Задолженность | Основной долг | Проценты | Иное | ..."
3. Из всех строк таблицы выбери САМУЮ ПОСЛЕДНЮЮ
   (нижнюю, актуальную по времени).
4. Возьми ТОЛЬКО числовые значения из колонок:
   — "Основной долг"
   — "Проценты"
   — "Иное"
5. Рассчитай:
   Сумма = Основной долг + Проценты + Иное
6. Используй ТОЛЬКО этот результат для поля "Сумма" в JSON.

Любые значения, находящиеся ВНЕ таблицы,
даже если они содержат слова "Сумма задолженности",
...
{{
  "Кредиты": [
    {{
      "Кредитор": "нормализованное название",
      "ИНН_кредитора": "10-значный ИНН из строки 'ИНН³'",
      "Вид": "тип кредита",
      "Дата_сделки": "ДД.ММ.ГГГГ",
      "Сумма_обязательства": "первоначальная сумма кредита",
      "Сумма": "значение из 'Общая задолженность'"
    }}
  ]
}}
""",
...
Пустые значения = null. Суммы ТОЛЬКО числами. Без комментариев.""",
        ...
        "паспорт_супруга": {
            "keywords": ["паспорт супруга", "паспорта супруга", "паспорт жены", "паспорта жены", "паспорт мужа", "паспорта мужа", "паспорт заинтересованного лица", "паспорта заинтересованного лица", "passport spouse"],
...
Если реквизит не найден, ставь null. Никаких дополнительных комментариев.""",
        ...
        "свидетельство_о_браке": {
            "keywords": ["свидетельство о заключении брака", "свидетельство о браке", "копия свидетельства о заключении брака", "копия свидетельства о браке", "запись акта о заключении брака", "брак", "супруг", "супруга", "загс"],
...
- В свидетельтве могут быть указаны прежние фамилии супругов до брака, чтобы извлечь актуальные фамилии после брака — в документе указывается "После заключения брака присвоены фамилии: мужу ... жене ..." - вот от туда ивзлеки присвоенные фамилии.
...
Если реквизит не найден, ставь null. Никаких дополнительных комментариев.""",
        ...
        "свидетельство_о_рождении": {
            "keywords": ["свидетельство о рождении", "свидетельств о рождении", "свидетельств о рождении детей", "свидетельство о рождении ребенка", "копия свидетельства о рождении", "копии свидетельств о рождении", "копии свидетельств о рождении детей",  "родился", "родилась", "запись акта о рождении", "ребенок", "загс"],
