...
Может быть две версии отчёта БКИ: старая и новая.
            
В старом отчете таблица "АКТИВНЫЕ ДОГОВОРЫ", а в новой "Действующие кредиты, займы, карты".

Нормализуй название банка, МФК, МКК, например: сбербанк - ПАО «Сбербанк», псб банк - ПАО «Банк ПСБ», Вэзбанкир ООО МФК - ООО МФК «Вэббанкир», МФК Юпитер 6 ООО - ООО МФК «Юпитер 6» и т.д.

Для КАЖДОЙ строки таблицы извлеки:
- Кредитор (точное название из колонки)
- В старой версии: Тип кредита/займа (из колонки "Вид обязательства" или аналогичной: Потребительский кредит, Кредитная карта, Микрозайм, Ипотека, Автокредит, Необеспеченный кредит, и т.д.).
    В новой версии "Источник (кредитор) / Вид обязательства" - в этой колонке указано название банка и вид кредита.
- В старой: Дата начала договора (из колонки "Дата начала договора").
    В новой: Дата начала обязательства (из колонки "Дата начала обязательства")
- В старой: Сумма обязательства по договору (ОБЯЗАТЕЛЬНО! из колонки "Сумма обязательства по договору" - первоначальная сумма кредита).
    В новой: Сумма / лимит обязательства (ОБЯЗАТЕЛЬНО! из колонки "Сумма / лимит обязательства" - первоначальная сумма кредита)
- В старой: Текущий долг = "Текущая задолженность" + "Текущая просрочка, сумма".
    В новой текущий долг = "Текущая просрочка (осн.долг, %, пени...)", то есть в новой версии уже всё включено в одну колонку "Текущая просрочка (осн.долг, %, пени...)".
            ...
            for data in data_list:
                ...
                for obj_item in objects:
                    ...
                    for право in owners:
                        ...
                        is_terminated = False
                        if дата_прекращения and дата_прекращения.lower() != "данные отсутствуют":
                             # Пытаемся понять, это дата или нет. Если просто текст "данные отсутствуют" - не дата.
                             # Если "03.12.2019" - дата.
                             if any(char.isdigit() for char in дата_прекращения):
                                 is_terminated = True
