...
Верни JSON строго в формате:
{
  ...
  "ФИО_супруга": "Иванов Иван Иванович",  # Только если есть штамп о браке! Полное ФИО супруга из штампа на стр. 7-8. Если нет брака или нечитаемо - null
  "Дата_рождения_супруга": "ДД.ММ.ГГГГ",  # Дата рождения супруга из штампа о браке. Если не указана в штампе - null
...
ВАЖНОЕ ЗАМЕЧАНИЕ! НЕ НУЖНО ОТДЕЛЬНО БРАТЬ "Общая задолженность" и "Просроченная задолженность" КАК ОТДЕЛЬНЫЕ КРЕДИТЫ. ЭТО ОДИН И ТОТ ЖЕ КРЕДИТ!!!
...
2. Ищи структуру кредитных записей даже без заголовков.

СНАЧАЛА найди ИНН должника на первой странице (раздел "СУБЪЕКТ КРЕДИТНОЙ ИСТОРИИ").

Найди таблицу "ДЕЙСТВУЮЩИЕ КРЕДИТНЫЕ ДОГОВОРЫ". Игнорируй "ЗАКРЫТЫЕ".

ВНИМАНИЕ: В отчете ОКБ есть "Краткая таблица" (сводка) и "Детальный раздел" (подробности по каждому кредиту ниже).
ДАННЫЕ БЕРИ ИЗ ДЕТАЛЬНОГО РАЗДЕЛА, ТАК КАК ТАМ БОЛЬШЕ ИНФОРМАЦИИ (ДАТЫ, ИНН КРЕДИТОРА).

Нормализуй название банка, МФК, МКК (см. примеры ниже).
ПРИМЕРЫ: "Сбербанк" -> "ПАО «Сбербанк»", "ВТБ" -> "Банк ВТБ (ПАО)", "Тинькофф" -> "АО «Т-Банк»".

Для КАЖДОГО кредита в ДЕТАЛЬНОМ РАЗДЕЛЕ извлеки:
   1. "Кредитор": Название банка/МФО.
   2. "ИНН_кредитора": Ищи строку "ИНН³" или просто "ИНН" в блоке "Сведения об источнике". (10 цифр).
   3. "Вид": Тип кредита (Потребительский, Карта, Микрозайм и т.д.).
   4. "Дата_сделки": Дата оформления договора (НЕ дата обновления информации!). Ищи "Дата возникновения обязательства".
   5. "Сумма_обязательства": Первоначальная сумма кредита/лимит карты. Ищи "Сумма и валюта обязательства".
   6. "Сумма" (Текущий долг): 
      - Ищи раздел "Сведения о сумме задолженности".
      - В нем найди поле "Общая сумма задолженности" (или просто "Общая задолженность", "Всего").
      - ЕСЛИ ЕСТЬ ПОЛЕ "Общая задолженность" — БЕРИ ЕГО! Оно включает всё (основной долг, проценты, штрафы).
      - ЕСЛИ НЕТ ОБЩЕЙ СУММЫ: Сложи "Основной долг" + "Проценты" + "Иное/Штрафы".
      - ВАЖНО: НЕ бери сумму из краткой таблицы, бери из детальной, чтобы не перепутать строки.

КРИТИЧЕСКИ ВАЖНО:
- НЕ СОЗДАВАЙ ДУБЛИКАТЫ ИЗ КРАТКОЙ ТАБЛИЦЫ И ДЕТАЛЬНОЙ. ИСПОЛЬЗУЙ ТОЛЬКО ДЕТАЛЬНУЮ.
- Если у должника 5 кредитов — верни 5 объектов.
- Один договор = один объект JSON.

СУДЕБНЫЕ АКТЫ (для дедупликации):
- В детальной таблице ищи раздел "Сведения о судебном споре".
- Если есть, извлеки "Номер судебного акта" и "Дата принятия".

JSON:
{{
  "ИНН_должника": "12 цифр",
  "Кредиты": [
    {{
      "Кредитор": "...",
      "ИНН_кредитора": "...",
      "Вид": "...",
      "Дата_сделки": "ДД.ММ.ГГГГ",
      "Сумма_обязательства": "число",
      "Сумма": "число (текущий долг)"
    }}
  ],
  "Судебные_акты": []
}}""",
...
ИНСТРУКЦИЯ ПО ОБРАБОТКЕ ЧАСТЕЙ ДОКУМЕНТА (БАТЧИ):
1. Если видишь таблицу "Действующие кредиты..." или просто список кредитов — ИЗВЛЕКАЙ ИХ!

В отчете ищи таблицу "Действующие кредиты, займы, карты" (Active Loans).

Нормализуй название банка (Сбербанк -> ПАО «Сбербанк», Тинькофф -> АО «Т-Банк», ВТБ -> Банк ВТБ (ПАО)).

Для КАЖДОЙ строки таблицы извлеки:
1. "Кредитор": Название из первой колонки.
2. "Вид": Тип кредита (Потребительский, Карта, Микрозайм). Обычно в колонке "Источник / Вид обязательства".
3. "Дата_сделки": Дата начала договора (Колонка "Дата начала обязательства").
4. "Сумма_обязательства": Первоначальная сумма/лимит. Колонка "Сумма / лимит обязательства".
5. "Сумма" (Текущий долг):
   - Ищи колонку "Текущая задолженность" или "Общая сумма задолженности".
   - Если такой нет, ищи колонку "Срочный основной долг" И колонку "Просроченная задолженность".
   - СУММА = "Срочный основной долг" + "Просроченная задолженность" (если они разделены).
   - Если есть колонка "Всего к оплате" или "Текущий долг (всего)" — бери её.
   - ВАЖНО: Не путать с "Сумма обязательства" (это сколько взяли, а не сколько должны сейчас).
...
{{
  "Кредиты": [
    {{
      ...
      "Вид": "...",
      "Дата_сделки": "ДД.ММ.ГГГГ",
      "Сумма_обязательства": "число (начальная сумма)",
      "Сумма": "число (текущий долг total)"
    }}
  ]
}}""",
        },
...
СНАЧАЛА найди ИНН должника на первой странице отчета (это 12-значное число физического лица). Указано в разделе "Субъект кредитной истории".

ОСНОВНОЙ АЛГОРИТМ:
1. Ищи блоки, начинающиеся с нумерации (1, 2, 3...) или с названия Банка/Кредитора.
2. В каждом блоке ищи подраздел "Сведения о счете" или "Обязательство".
3. В таблице "Сведения о счете" (или аналогичной):
   - "Вид счета (договора)" -> Тип кредита (Потребительский кредит, Кредитная карта, Микрозайм и т.д.)
   - "Дата открытия" -> Дата сделки (Дата_сделки)
   - "Валюта и сумма обязательства" (или "Лимит кредитования") -> Первоначальная сумма (Сумма_обязательства). Если это кредитная карта, бери ЛИМИТ.
   - "Код причины закрытия счета" -> Проверь статус. Если "Счет закрыт..." или "Надлежащее исполнение..." - ПРОПУСКАЙ. Если "Счет открыт" или "Активный" - БЕРИ.
4. В таблице "Состояние счета" или "Задолженность":
   - "Общая сумма задолженности" (или просто "Сумма") -> Текущий долг (Сумма).
   - Если есть разбивка (основной долг, проценты) - бери ОБЩУЮ СУММУ.

ПРО БАНКИ:
- Нормализуй названия: "Сбербанк России" -> "ПАО «Сбербанк»", "ВТБ 24" -> "Банк ВТБ (ПАО)", "Тинькофф" -> "АО «Т-Банк»".
- ИНН Кредитора часто указан рядом с названием банка или в разделе "Сведения о кредиторе". ПОСТАРАЙСЯ НАЙТИ ЕГО.

ВЕРНИ ТОЛЬКО ВАЛИДНЫЙ JSON БЕЗ ДОПОЛНИТЕЛЬНОГО ТЕКСТА!

JSON (СТРОГО этот формат):
{{
  "ИНН_должника": "12-значное число",
  "Кредиты": [
    {{
      "Кредитор": "название (нормализованное)",
      "ИНН_кредитора": "10 цифр (если есть)",
      "Вид": "тип кредита",
      "Дата_сделки": "ДД.ММ.ГГГГ",
      "Сумма_обязательства": "число (первоначальная)",
      "Сумма": "число (текущий остаток долга)"
    }}
  ]
}}""",
        },
                ...
                if owner_fio:
                    for право in правообладатели:
                        ...
                        if owner_fio and фио:
                            ...
                            owner_parts = owner_fio.lower().split()
                            право_parts = фио.lower().split()
                            
                            # Проверяем совпадение хотя бы Фамилии (первое слово)
                            if owner_parts and право_parts and owner_parts[0] == право_parts[0]:
                                owner_found = True
                                owner_data = право
                                break
                            # Также проверяем полное вхождение (на случай если порядок разный)
                            elif owner_fio.lower() in фио.lower():
                                owner_found = True
                                owner_data = право
                                break
                    if not owner_found:
...
В разделе "ДЕЙСТВУЮЩИЕ КРЕДИТНЫЕ ДОГОВОРЫ" есть краткая таблица с информацией по кредитам, а дальше уже перечисление кредитов 1. 2. 3. и т.д. с детальной информацией по каждому кредиту, не нужно из краткой таблицы пытаться брать данные, сразу ищи детальный раздел НИЖЕ с подробной информацией по каждому кредиту.
...
В ДЕТАЛЬНОМ РАЗДЕЛЕ НИЖЕ, ПОД КРАТКОЙ ТАБЛИЦЕЙ, для КАЖДОГО кредитора извлеки:
   ...
   - Обязательно извлеки тип кредита/займа из колонки "Вид займа (кредита)".
   - Сложи "Основной долг" + "Проценты" + "Иное" из детального раздела НИЖЕ в таблице "Сведения о сумме задолженности" - это и есть "Общая задолженность" из краткой таблицы

КРИТИЧЕСКИ ВАЖНО ПРО ИЗВЛЕЧЕНИЕ СПИСКА КРЕДИТОВ:
- Ты должен извлечь АБСОЛЮТНО ВСЕ КРЕДИТЫ из таблицы "ДЕЙСТВУЮЩИЕ КРЕДИТНЫЕ ДОГОВОРЫ"!
- Если в таблице 8 строк - верни 8 объектов в JSON!
- Если у должника несколько кредитов в одном банке (например, 5 кредитов в "АО «ГПБ»") — ЭТО РАЗНЫЕ КРЕДИТЫ! НЕ ОБЪЕДИНЯЙ ИХ!
- Важно: Кредиты могут быть разного типа (Овердрафт, Потребительский, Ипотека) — это разные договоры!
- Даже если тип кредита одинаковый (например, два "Иной необеспеченный заем") — это разные кредиты!
- Создай отдельный элемент массива для КАЖДОЙ строки таблицы.
- НЕ группируй кредиты по названию банка или типу. Один договор = один объект в JSON.
- Точно перепиши суммы и даты для каждого кредита, чтобы они не перепутались.

КРИТИЧЕСКИ ВАЖНО ПРО ДОЛГ В ОКБ:
- Используй ТОЛЬКО "Общая задолженность" из краткой таблицы "ДЕЙСТВУЮЩИЕ КРЕДИТНЫЕ ДОГОВОРЫ"
- Берем общую сумму долга из столбца "Общая задолженность" в краткой таблице.
- Запрещено брать сумму из "Просроченная задолженность" и делать из нее общую задолженность!
- "Общая задолженность" УЖЕ ВКЛЮЧАЕТ просрочку, проценты и всё остальное!
- "Просроченная задолженность" - Его не трогай, НЕ используй его!
...
- Когда начинается раздел "ЗАКРЫТЫЕ КРЕДИТНЫЕ ДОГОВОРЫ", прекращай сбор данных.
- В краткой таблице "Статус платежа" не дата возникновения обязательства. Нас интересует "Дата возникновения обязательства" в подробной таблице.

Нормализуй название банка, обрати внимание на список примеров выше. "ГПБ (АО)" -> "АО «ГПБ»".

СУДЕБНЫЕ АКТЫ (для дедупликации с постановлениями приставов):
- В детальной таблице каждого кредита может быть подраздел "Сведения о судебном споре или требовании по обязательству"
- Если есть этот раздел, найди строку "Номер судебного акта и дата принятия" (пример: "2-528/2025 / 16 апреля 2025")
- Извлеки номер дела и дату в формате "номер дата" (пример: "2-528/2025 16.04.2025")
- Если таких актов несколько - извлеки ВСЕ
- Если нет такого раздела - массив "Судебные_акты" будет пустым
...
JSON:
{{
  "ИНН_должника": "12-значное число физического лица на первой странице",
  "Кредиты": [
    {{
      "Кредитор": "название (нормализованное)",
  ...
  "Судебные_акты": [
    {{
      "Номер_дела": "2-528/2025",
      "Дата_акта": "16.04.2025"
    }}
  ]
}}"""
...
Для КАЖДОЙ строки краткой таблицы извлеки:
- Кредитор (точное название из колонки)
- "Источник (кредитор) / Вид обязательства" - в этой колонке указано название банка и вид кредита.
- Дата начала обязательства (из колонки "Дата начала обязательства")
- Сумма / лимит обязательства (ОБЯЗАТЕЛЬНО! из колонки "Сумма / лимит обязательства" - первоначальная сумма кредита)
- Текущий долг = "Срочный основной долг", то есть уже всё включено в одну колонку "Текущая просрочка (осн.долг, %, пени...)".

ВАЖНО: 
- "Сумма / лимит обязательства" - это первоначальная сумма кредита, НЕ текущий долг!
- Поле в JSON ОБЯЗАТЕЛЬНО должно называться "Кредиты", НЕ "Договоры"!
- В БКИ отчёте НЕТ ИНН кредитора, поэтому НЕ извлекай его!
- Извлекай тип кредита из колонки "Источник (кредитор) / Вид обязательства", пример:
Столбец "Источник (кредитор) / Вид обязательства":
"Банк: ПАО Сбербанк
Кредитная линия/карта" - Тип кредита: Кредитная линия/карта
...
СУДЕБНЫЕ АКТЫ (для дедупликации с постановлениями приставов):
- В детальной таблице каждого кредита может быть подраздел "Сведения о судебном споре или требовании по обязательству по договору"
- Если есть этот раздел, найди строку "Дата принятия акта и номер акта" (пример: "08.11.2023 2-5856/2023")
- Извлеки дату и номер дела в формате: Дата_акта = "08.11.2023", Номер_дела = "2-5856/2023"
- Если таких актов несколько - извлеки ВСЕ
- Если нет такого раздела - массив "Судебные_акты" будет пустым
...
{{
  "Кредиты": [
    {{
      ...
      "Вид": "тип кредита (Потребительский кредит/Кредитная карта/Микрозайм/Ипотека/Автокредит/другое)",
      "Дата_сделки": "дата",
      "Сумма_обязательства": "первоначальная сумма из колонки 'Сумма / лимит обязательства'",
      "Сумма": "Срочный основной долг" - НЕ НАДО СЮДА СТАВИТЬ 'Сумма / лимит обязательства', СЮДА НАДО "Срочный основной долг"
    }}
  ],
  "Судебные_акты": [
    {{
      "Дата_акта": "08.11.2023",
      "Номер_дела": "2-5856/2023"
    }}
  ]
}}"""
