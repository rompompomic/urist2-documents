...
4. Штампы о семейном положении (стр. 14-15):
        КРИТИЧЕСКОЕ ПРАВИЛО:
        Если ты видишь ДВА ШТАМПА (один ниже другого или рядом):
        1. Первый штамп - "Зарегистрирован брак".
        2. Второй штамп - "Брак расторгнут" (или "Свидетельство о расторжении брака").
        ИТОГОВЫЙ СТАТУС -> "Разведен" / "Разведена" (или "Не женат"/"Не замужем").
        НИКОГДА не пиши "Женат/Замужем", если есть штамп о разводе!
        
        Если есть штамп о браке, а после него расположен следующий штамп, который: нечитаемый, смазан, криво поставлен, частично виден, невозможно достоверно определить как брак или развод, то НЕ ИСПОЛЬЗУЙ предыдущий штамп о браке как актуальный статус. В этом случае итоговый статус ставь: "Не женат" / "Не замужем"
        Это правило имеет приоритет выше, чем правило о читаемом штампе брака.
        - ВНИМАНИЕ: Если несколько штампов — выбери ПОСЛЕДНИЙ (актуальный статус).
        - Если был брак, затем развод — статус "Разведен/Разведена".
        - Если штамп перечёркнут диагональными линиями — это официальный признак аннулирования записи в паспортах РФ -> статус "Не женат".
        
        СУПЕР-КРИТИЧЕСКОЕ ПРАВИЛО ПРО ВТОРОЙ ШТАМП ИЛИ РАЗВОД:
        Если ты видишь нормальный штамп "Зарегистрирован брак", а ПОСЛЕ него (ниже или сбоку) или НА СЛЕДУЮЩЕЙ СТРАНИЦЕ есть:
        1. ВТОРОЙ штамп любой степени читаемости
        2. Любое синее, черное, красное или фиолетовое СМАЗАННОЕ ПЯТНО прямоугольной формы
        3. Нечитаемый кусок текста, похожий на печать
        4. Явный штамп "Брак расторгнут"
        ТО ТЫ ОБЯЗАН СЧИТАТЬ, ЧТО ЭТО РАЗВОД!
        В этом случае СТРОГО:
        - "Семейное_положение": "Разведен" (или "Не замужем")
        - "ФИО_супруга": null
        - "Дата_рождения_супруга": null
        
        НИКОГДА НЕ ВОЗВРАЩАЙ СТАТУС "Женат/Замужем" И ДАННЫЕ СУПРУГА, ЕСЛИ ЕСТЬ ХОТЯ БЫ МАЛЕЙШИЙ НАМЕК НА ВТОРОЙ ШТАМП!
        ЛУЧШЕ ОШИБИТЬСЯ И НАПИСАТЬ "НЕ ЖЕНАТ", ЧЕМ НАПИСАТЬ "ЖЕНАТ" ПРИ БРАКЕ, КОТОРЫЙ РАСТОРГНУТ!
        
        Пример:
        - Вижу: "Зарегистрирован брак с Ивановым..." (четко).
        - Вижу ниже: какое-то пятно или смазанный текст прямоугольной формы.
        - ВЫВОД: Это развод. Статус -> "Разведен". ФИО супруга -> null.

        - КРИТИЧЕСКИ ВАЖНО: Если есть штамп о браке (и абсолютно чистое пространство после него, без малейших следов других печатей) - извлеки ФИО супруга и дату рождения супруга из штампа!
        ...
        - Короче, если первый штамп брак, второй непонятная каша какая-то — итоговый статус "Не женат"/"Не замужем", а ФИО_супруга - ОБЯЗАТЕЛЬНО null!
5. Печати о детях (стр. 16-17):
        - ВНИМАТЕЛЬНО изучи раздел "Дети".
        - Это могут быть печати или рукописные записи.
        - Извлеки КАЖДОГО ребенка.
        - Формат записи обычно: "Пол", "ФИО", "Дата рождения".
        - Если фамилии у ребенка в записи нет (только Имя Отчество), а в паспорте владельца фамилия указана -> используй фамилию владельца паспорта для ребенка.
        - Собери ВСЕХ, кого найдешь.
        - Пример: "Иванов Иван Иванович 20.10.2015"
...
JSON:
{
        ...
        "Дата_рождения": "ДД.ММ.ГГГГ",
        "СНИЛС": "...",
        "Текущая_работа": "Название организации, Должность" или null (если уволен), точку в конце не ставь,
        "Записи": [
                {
                        "Работодатель": "...",
                        "РегНомер": "...",
...
Пустые значения = null. Никаких дополнительных комментариев.""",
    ...
    def normalize_quotes(text: str) -> str:
        ...
        Алгоритм:
        1. Сводим все кавычки к обычным двойным (")
        2. Заменяем " на « если они стоят в начале слова (после пробела, скобки, начала строки)
        3. Остальные " заменяем на »
        Это позволяет корректно обрабатывать вложенные кавычки: "А "Б"" -> «А «Б»»
        ...
        # 1. Сводим всё к обычным двойным кавычкам
        # Заменяем елочки, лапки, одиночные кавычки и прочий мусор
        cleaned = re.sub(r'[\"\'«»„“]', '"', text)
        
        # 2. Открывающие кавычки: в начале строки, после пробела, скобки, тире, дефиса, новой строки
        # Группа \1 сохраняет разделитель
        with_opening = re.sub(r'(^|[\s\(\-])"', r'\1«', cleaned)
        
        # 3. Всё что осталось - закрывающие
        result = with_opening.replace('"', '»')
  ...
  "Название 2": "Нормализованное название 2"
    ...
    def extract_employers(records: Dict[str, Any]) -> str:
        ...
        if last_entry and last_entry.get("Событие") != "Увольнение":
            ...
            if employer:
                ...
                # Нормализуем кавычки
                workplace = DocumentProcessor.normalize_quotes(workplace)
    ...
    def determine_children_status(children: Optional[List[Dict[str, Any]]], data_map: Dict[str, List[Dict[str, Any]]] = None) -> str:
        ...
        def add_minor(name_raw: str, dob_str: str):
            ...
            try:
                ...
                name_clean = name_raw.strip() if name_raw else "ФИО не указано"
    ...
    def get_marital_status_key(passport: Dict[str, Any], data_map: Dict[str, List[Dict[str, Any]]] = None) -> str:
        ...
        not_married = (
            ...
            "разведен" in marital_status or
            "расторгнут" in marital_status
        )
        
        if not_married:
            # Даже если есть ФИО супруга (бывшего), статус должен быть "не в браке"
            return "не в браке"
    ...
    def convert_to_genitive_initials(full_name: str) -> str:
        ...
        prompt = (
            ...
            "ВАЖНО ПРО ФАМИЛИИ НА СОГЛАСНЫЙ (Сергак, Жук, Ким):\n"
            "- ЖЕНЩИНА -> Фамилия НЕ склоняется! (Сергак М.И.)\n"
            "- МУЖЧИНА -> Фамилия СКЛОНЯЕТСЯ! (Сергака И.И.)\n\n"
            f"ФИО: {full_name}"
        )
        try:
            response = client.chat.completions.create(
    ...
    def convert_to_genitive_full(full_name: str) -> str:
        ...
        prompt = (
            ...
            " Верни только результат без пояснений и дополнительных знаков.\n\n"
            "ВАЖНО ПРО ФАМИЛИИ НА СОГЛАСНЫЙ (Сергак, Жук, Ким и т.д.):\n"
            "- Если это ЖЕНЩИНА (Мария, Анна...) -> Фамилия НЕ склоняется! (Марии Сергак)\n"
            "- Если это МУЖЧИНА (Иван, Петр...) -> Фамилия СКЛОНЯЕТСЯ! (Ивана Сергака)\n\n"
            f"ФИО: {full_name}\n"
    ...
    def convert_to_dative_initials(full_name: str) -> str:
        ...
        prompt = (
            ...
            "ВАЖНО ПРО ФАМИЛИИ НА СОГЛАСНЫЙ (Сергак, Жук, Ким):\n"
            "- ЖЕНЩИНА -> Фамилия НЕ склоняется! (Сергак М.И.)\n"
            "- МУЖЧИНА -> Фамилия СКЛОНЯЕТСЯ! (Сергаку И.И.)\n\n"
            f"ФИО: {full_name}"
        )
        try:
            response = client.chat.completions.create(
    ...
    def convert_to_dative_full(full_name: str) -> str:
        ...
        prompt = (
            ...
            " Верни только результат без пояснений и дополнительных знаков.\n\n"
            "ВАЖНО ПРО ФАМИЛИИ НА СОГЛАСНЫЙ (Сергак, Жук, Ким):\n"
            "- ЖЕНЩИНА -> Фамилия НЕ склоняется! (Марии Сергак)\n"
            "- МУЖЧИНА -> Фамилия СКЛОНЯЕТСЯ! (Ивану Сергаку)\n\n"
            f"ФИО: {full_name}\n"
    ...
    def convert_to_accusative_full(full_name: str) -> str:
        ...
        prompt = (
            ...
            " Верни только результат без пояснений и дополнительных знаков.\n\n"
            "ВАЖНО ПРО ФАМИЛИИ НА СОГЛАСНЫЙ (Сергак, Жук, Ким):\n"
            "- ЖЕНЩИНА -> Фамилия НЕ склоняется! (Марию Сергак)\n"
            "- МУЖЧИНА -> Фамилия СКЛОНЯЕТСЯ! (Ивана Сергака)\n\n"
            f"ФИО: {full_name}\n"
            "Пример: Иванов Иван Иванович → Иванова Ивана Ивановича"
        )
        try:
...
1. "субъект" - это область, край, республика (например: "Курганская область").
2. "район" - это административный район области (например: "Альменевский район").
3. "город" - заполняй ТОЛЬКО если это город (например: "Челябинск"). Если это село, деревня, поселок - оставляй "город" пустым!
4. "населенный_пункт" - это село, деревня, поселок, пгт (например: "с. Альменево", "с Бродокалмак"). Если перед названием стоит "с", "село", "д", "деревня", "пгт", "пос" - пиши это сюда.
5. "улица" - улица, проспект, переулок (например: "ул. Базарная", "пер. Северный"). ЕСЛИ в адресе есть название (например: "Базарная", "Мира"), но нет слова "улица" или "ул", ВСЕ РАВНО пиши это в "улица" (добавляй "ул." перед названием). Главное - не путай с населенным пунктом.
6. "дом" - просто номер (например: "23", "9").
7. "корпус" - если есть.
8. "квартира" - если есть. ВНИМАНИЕ: СТРОГО оставляй пустым, если в исходном адресе явно не написано "кв", "квартира" или нет явного номера квартиры! Не придумывай "кв. 1", если это частный дом!
...
Пример 1: "Курганская область, Альменевский район, с. Альменево, пер. Северный, д. 23"
Должно быть:
{{
  "субъект": "Курганская область",
  "район": "Альменевский район",
  "город": "",
  "населенный_пункт": "с. Альменево",
  "улица": "пер. Северный",
  "дом": "23",
  "корпус": "",
  "квартира": ""
...
Пример 2: "Россия, Челябинская область, Красноармейский район, с Бродокалмак, Базарная, д. 9"
Должно быть:
{{
  "субъект": "Челябинская область",
  "район": "Красноармейский район",
  "город": "",
  "населенный_пункт": "с. Бродокалмак",
  "улица": "ул. Базарная",
  "дом": "9",
  "корпус": "",
  "квартира": ""
