...
Справки должны быть отсортированы по году (от старых к новым). Если какого-то раздела нет, ставь null. Никаких пояснений.""",
        ...
        "трудовая": {
            "keywords": ["трудов", "сзв", "трудовой деятельности", "сведения о трудовой", "трудовая книжка", "сзв-стаж", "сзв-м", "сфр", "пфр", "сведения о трудовой деятельности", "ЭТК"],
            "prompt": """Документ СФР "Сведения о трудовой деятельности" (форма СТД-Р/СТД-ПФР).

КРИТИЧЕСКИ ВАЖНО - ИЗВЛЕКАЙ ТОЛЬКО ТЕКУЩЕЕ МЕСТО РАБОТЫ:
- Найди ПОСЛЕДНЮЮ запись в таблице (самая свежая дата)
- Если последняя запись = "Увольнение" → человек НЕ работает, Текущая_работа = null
- Если последняя запись = "Приём" или "Перевод" → человек РАБОТАЕТ в этой организации
- НЕ извлекай все записи из таблицы, только последнюю для определения текущего работодателя!
- НЕ извлекай периоды работы до 2019 года!
...
- Найди ПОСЛЕДНЮЮ запись в таблице и определи текущий статус:
  • Если последнее событие "Приём" или "Перевод" → укажи работодателя и должность в "Текущая_работа"
  • Если последнее событие "Увольнение" → Текущая_работа = null
...
JSON:
{
        ...
        "Дата_рождения": "ДД.ММ.ГГГГ",
        "СНИЛС": "...",
        "Текущая_работа": "Название организации, Должность" или null (если последняя запись - увольнение)
}
...
- Один банк может иметь НЕСКОЛЬКО счетов (например: Сбербанк текущий счет в Москве И Сбербанк вклад в Челябинске И Сбербанк счет по вкладу)
- НЕ останавливайся после первых нескольких счетов - продолжай до конца таблицы!

Фильтр по статусу (включай только АКТИВНЫЕ):
- ✅ ВКЛЮЧАЙ: "Открыт", "Активен", "Действующий", "Текущий счет", "Счет по вкладу", "Открыт", "Предоставлено право использования"
- ❌ НЕ ВКЛЮЧАЙ: "Закрыт", "закрыт", "Расторгнут", "Закрыт по инициативе клиента", "Прекращено право использования"
...
КРИТИЧЕСКИ ВАЖНО: Отличай ВЫПИСКУ от УВЕДОМЛЕНИЯ ОБ ОТСУТСТВИИ!

УВЕДОМЛЕНИЕ ОБ ОТСУТСТВИИ (имущества НЕТ):
- Есть фразы: "отсутствует запрошенная Вами информация", "в ЕГРН отсутствуют сведения", "данные не найдены"
- Таблица содержит ТОЛЬКО "Вид запрошенной информации" и "Правообладатель" БЕЗ объектов
- НЕТ кадастровых номеров, адресов объектов, площади, стоимости
- В этом случае верни: {"Есть_недвижимость": false, "Причина": "Уведомление об отсутствии сведений в ЕГРН"}

ВЫПИСКА С ИМУЩЕСТВОМ (имущество ЕСТЬ):
- Есть КОНКРЕТНЫЕ объекты с кадастровыми номерами (формат XX:XX:XXXXXXX:XXXX)
- Указаны характеристики: адрес, площадь, назначение, стоимость
- Есть раздел "Правообладатели" с ФИО владельцев
- В этом случае верни: {"Есть_недвижимость": true, ...данные объектов...}
...
4. Кадастровая стоимость(Может не быть)
