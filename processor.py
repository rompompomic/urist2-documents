        ...
        for кредитор_normalized, credits_list in sorted_creditors:
            ...
            for contract_idx, credit in enumerate(credits_list):
                ...
                row_data = {
                    ...
                    "ИНН_кредитора": инн_display,  # Показываем на всех строках (для объединения ячеек)
                    "Инн_кредитора": инн_display,  # Алиас для шаблона
                    "ИНН": инн_display,            # Алиас для шаблона (опасно, может пересечься с должником, но в таблице usually ok)
                ...
                # ЛОГИРУЕМ что реально добавили
                if is_first_contract:
                    print(f"[DEBUG_TABLE_ROW] Добавлена строка 1.{counter}: Кредитор='{row_data['Кредитор']}', ИНН='{row_data['ИНН_кредитора']}'")
