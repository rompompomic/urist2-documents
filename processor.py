...
{{
  ...
  "Название 2": "Нормализованное название 2"
    ...
    def format_vehicles_table(гибдд_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Объединяем все ТС одного типа в один элемент для шаблона
        # Чтобы в шаблоне цикл выполнился один раз и заголовок не дублировался
        def combine_vehicles(vehicles_list):
            """Объединяет список ТС в один элемент с многострочными полями"""
            if not vehicles_list:
                return []
            
            combined = {
                "авто_пп": "\n".join(v["авто_пп"] + ")" for v in vehicles_list),
                "Идентификационный_номер": "\n".join(v["Идентификационный_номер"] for v in vehicles_list),
                "Вид_собственности": "\n".join(v["Вид_собственности"] for v in vehicles_list),
                "Место_нахождения": "\n".join(v["Место_нахождения"] for v in vehicles_list),
                "Стоимость": "\n".join(v["Стоимость"] for v in vehicles_list),
                "Сведения_о_залоге": "\n".join(v["Сведения_о_залоге"] for v in vehicles_list),
                "Вид_марка_модель": "\n".join(v["Вид_марка_модель"] for v in vehicles_list),
            }
            return [combined]

        return {
            "автомобили": combine_vehicles(автомобили),
            "грузовики": combine_vehicles(грузовики),
            "мото": combine_vehicles(мото),
            "сельхоз": combine_vehicles(сельхоз),
            "водный": combine_vehicles(водный),
            "воздушный": combine_vehicles(воздушный),
